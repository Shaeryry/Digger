...
std::vector<glm::vec2> Pathfinder::FindPath(TerrainComponent* terrain, const glm::vec2& startPixel, const glm::vec2& endPixel, int tileSize)
{
    Rinigin::Physics& physics{ Rinigin::Physics::GetInstance() };
    const glm::ivec2 start = glm::ivec2(startPixel.x / tileSize, startPixel.y / tileSize);
    const glm::ivec2 end = glm::ivec2(endPixel.x / tileSize, endPixel.y / tileSize);
    ...
    while (!openSet.empty())
    {
        ...
        if (current->pos == end)
        {
            ...
            for (Node* node = current; node; node = node->parent) {
                path.emplace_back(glm::vec2(node->pos.x * tileSize, node->pos.y * tileSize));
            }
        ...
        for (const glm::ivec2& neighbor : GetNeighbors(current->pos))
        {
            ...
            glm::vec3 bounds = glm::vec3(tileSize*.5f, tileSize*.5f, 0);
            glm::vec3 worldPos = glm::vec3(
                (neighbor.x + 0.5f) * tileSize,
                (neighbor.y + 0.5f) * tileSize,
                0
            );
